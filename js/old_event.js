...
function makeModal() {
  ...
  if (userReq && endingReq && ecoReq) {
    // 정보를 다 받아왔으면 실행함
    if (userInfo.day > userInfo.event_day) {
      // 아직 이벤트를 하지 않았으면
...
function endingRequest() {
  var xhr = new XMLHttpRequest();
  xhr.open("GET", "../php/getEndingInfo.php");
  xhr.send();
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      let json = JSON.parse(xhr.responseText);
...
/* 문화를 바꿔가는 그리스도인 엔딩 */
function culture() {
  if (userInfo["day"] >= 3 && userInfo["day"] <= 6) {
    // 3~6일차
    if (endingInfo["culDay"] == 0) {
      // 문화 1단계 발생한적 X
      eventList.push({ id: "event_23", ending: "문화", condition: 60 }); // event_23 : 문화 1단계
    } else if (endingInfo["culDay"] != 0) {
      // 한번이라도 문화 1단계 발생한적 O
      if (endingInfo["culStage"] == 0) {
        // 문화 1단계 선택 X
        eventList.push({ id: "event_23", ending: "문화" });
      } else if (endingInfo["culStage"] == 1) {
        // 문화 1단계 선택 O (아직 5일 안지난 상태)
        // pass;
      }
    }
  } else if (userInfo["day"] >= 7) {
    // 7일차 ~
    if (endingInfo["culDay"] == 0) {
      // 한번도 1단계 발생한적 X
      // pass;  //(문화 엔딩 소멸)
      ...
      if (endingInfo["culStage"] == 0) {
        ...
        // (1단계 선택 O) and (2단계 선택 X) 상태
        if (userInfo["day"] < endingInfo["culDay"] + 5) {
          // 선택 후 1~4일째
          // pass
        } else if (userInfo["day"] == endingInfo["culDay"] + 5) {
          // 선택 후 5일째
      ...
      } else if (endingInfo["culStage"] == 2) {
        // (2단계 선택 O) 상태 and (3단계 선택 X) 상태
        if (userInfo["day"] < endingInfo["culDay"] + 5) {
          // 선택 후 1~4일째
          // pass
        } else if (userInfo["day"] == endingInfo["culDay"] + 5) {
      ...
      } else if (endingInfo["culStage"] == 3) {
        // (3단계 선택 O) and (4단계 선택 X) 상태
        if (userInfo["day"] < endingInfo["culDay"] + 5) {
          // 선택 후 1~4일째
          // pass
        } else if (userInfo["day"] == endingInfo["culDay"] + 5) {
...
/* 우리 학교는 환경지킴이 엔딩 */
function school() {
  if (userInfo["day"] >= 4 && userInfo["day"] <= 7) {
    // 4~7일차
    if (endingInfo["envDay"] == 0) {
      // 환경지킴이 1단계 발생한적 X
    ...
    } else if (endingInfo["envDay"] != 0) {
      // 한번이라도 환경지킴이 1단계 발생한적 O
      // pass
    }
  } else if (userInfo["day"] >= 8) {
    // 8일차 ~
    if (endingInfo["envDay"] == 0) {
      // 한번도 1단계 발생한적 X
      ...
      if (endingInfo["envStage"] == 0) {
        // 1단계 선택 X 상태
        if (userInfo["day"] < endingInfo["envDay"] + 7) {
          // 선택 후 1~6일째
          // pass
        } else if (userInfo["day"] == endingInfo["envDay"] + 7) {
          // 선택 후 7일째
        ...
        if (userInfo["day"] < endingInfo["envDay"] + 7) {
          ...
          if (order == 0) {
            eventList.push({
              ...
              ending: "환경지킴이",
              condition: 100,
            }); // event_33 : 환경지킴이 2단계 - 환경 물품 기부
          } else if (order == 1) {
            eventList.push({
              id: "event_34",
              ending: "환경지킴이",
              condition: 100,
            }); // event_34 : 환경지킴이 2단계 - 화장실 리모델링
          }
        }
      }
    }
...
/* 청지기 가정 엔딩 */
function home() {
  if (endingInfo["homeDay"] == 0) {
    // 한번도 1단계 발생한적 X
    eventList.push({ id: "event_35", ending: "청지기" }); // event_35 : 청지기 1단계
  } else if (endingInfo["homeDay"] != 0) {
    // 한번이라도 1단계 발생한적 O

    if (endingInfo["homeStage"] == 0) {
      // (1단계 선택 X) 상태
      if (userInfo["day"] < endingInfo["homeDay"] + 5) {
        // 선택 후 1~4일째
        // pass
      } else if (userInfo["day"] == endingInfo["homeDay"] + 5) {
        // 선택 후 5일째
        eventList.push({ id: "event_35", ending: "청지기", condition: 100 }); // event_35 : 청지기 1단계
      }
    } else if (endingInfo["homeStage"] == 1) {
      // (1단계 선택 O) and (2단계 선택 X) 상태
      if (userInfo["day"] < endingInfo["homeDay"] + 5) {
        // 선택 후 1~4일째
        // pass
    ...
    } else if (endingInfo["homeStage"] == 2) {
      // (2단계 선택 O) and (3단계 선택 X) 상태
      if (userInfo["day"] < endingInfo["homeDay"] + 5) {
        // 선택 후 1~4일째
        // pass
      } else if (userInfo["day"] == endingInfo["homeDay"] + 5) {
    ...
    } else if (endingInfo["homeStage"] == 3) {
      // (3단계 선택 O) and (4단계 선택 X) 상태
      if (userInfo["day"] < endingInfo["homeDay"] + 5) {
        // 선택 후 1~4일째
        // pass
...
function selectOne(list) {
  ...
  /* 확률 처리 */
  if (prob_other_list.length == 1) {
    // 다른 확률 가지는 원소 한개 (20% 또는 60%)

    const rand_0_99 = Math.floor(Math.random() * 100);
    console.log("뽑힌 숫자: ", rand_0_99);

    if (rand_0_99 <= probability_list[0]) {
      // (20% 또는 60%)
      ...
      selected_id =
        prob_no_list[Math.floor(Math.random() * prob_no_list.length)];
    }
  } else if (prob_other_list.length == 2) {
    // 다른 확률 가지는 원소 두개 (20%, 60%)
    ...
    } else if (21 < rand_0_99 < probability_list[1]) {
      // 20% ~ 60%, 뽑힌 숫자가 21~60이면
      selected_id = prob_other_list[1];
    } else if (rand_0_99 >= probability_list[1]) {
      // 60% ~ , 뽑힌 숫자가 61~100 이면
      selected_id =
        prob_no_list[Math.floor(Math.random() * prob_no_list.length)];
    }
  ...
  if (prob_100_list.length > 1) {
    // 100% 확률 가지는 원소가 여러개
    return prob_100_list[Math.floor(Math.random() * prob_100_list.length)]; // 100% 확률 가지는 원소 중 랜덤 선택해서 반환
  } else if (prob_100_list.length == 1) {
    // 100% 확률 가지는 원소가 한개
    return prob_100_list[0];
  } else if (prob_100_list.length == 0 && prob_other_list.length != 0) {
    // 100% 확률 가지는 원소가 없음 & 다른 확률 가지는 원소 존재
    console.log("return selected_id;");
    return selected_id;
  } else if (prob_100_list.length == 0 && prob_other_list.length == 0) {
    // 100% 확률 가지는 원소가 없음 & 다른 확률 가지는 원소 없음
