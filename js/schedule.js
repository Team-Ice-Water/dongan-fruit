...
function checkHealth() {
  var health = parseInt(
    document.getElementById("health").getAttribute("aria-valuenow")
  );
  if (health <= 50) {
    if (isFill["count"] == 2) {
      // 체력 50 이하. 2개 채워짐
...
function sendValue(findThis) {
  for (let value of changeValue) {
    if (value["id"] == findThis) {
      // findThis는 선택된 것의 name 속성 (물잠그기->'savewater')
...
function sendSchedule() {
  ...
  if (isVacation) {
    ...
    // 삭제버튼으로 인해 채워짐이 불규칙적이라, 모든 경우를 나열해줌
    if (dawn) {
      sendValue(dawn);
      if (am) {
        sendValue(am);
