...
function infoRequest() {
  ...
  xhr.open("GET", "../php/getEnding.php");
  xhr.send();
...
function makeCard() {
  // 엔딩 모음 페이지(collection.html)에서 사용할 함수

  if (characterInfo.length == 0) {
    var text = $(
      '<p class="fs-4 mx-auto mt-5 text-muted">아직 엔딩을 본 캐릭터가 없습니다.</p>'
    );
    ...
    for (let i = 0; i < characterInfo.length; i++) {
      ...
      var item = $(
        `
          ...
          characterInfo[i].name +
          `<br/>
                    <span class="fs-5 highlight">` +
          changeTxt(ending) +
          `</span></p>
                </div>
            </div>`
...
function setImg() {
  // 모든 엔딩 페이지(allEnding.html)에서 사용할 함수
  if (characterInfo.length > 0) {
    for (let i = 0; i < characterInfo.length; i++) {
      var ending = characterInfo[i].ending_type;
